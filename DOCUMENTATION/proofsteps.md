...
| `rightForall` | 1    | $$\frac{\Gamma \vdash A[x := y], \Delta}{\Gamma \vdash \forall x. A, \Delta}$$ | `i:Int`: Index of $\forall x. A$ on the right <br> `y:Var`: Variable in place of $x$ in the premise. | $y$ should not be free in the resulting sequent. |
