...
fof(f1, plain, [(sf(sf(sf(cemptySet))) = cemptySet)] --> [(cemptySet = sf(sf(sf(cemptySet))))], inference(rightSubst, [status(thm), 0, 1, $fof((cemptySet = HOLE)), 'HOLE'], [f0])).
fof(f2, plain, [] --> [(cemptySet = sf(sf(sf(cemptySet))))], inference(cut, [status(thm), 0, 0], [a1, f1])).
fof(f3, plain, [(sf(sf(cemptySet)) = cemptySet)] --> [(cemptySet = sf(cemptySet))], inference(rightSubst, [status(thm), 0, 0, $fof((cemptySet = sf(HOLE))), 'HOLE'], [f2])).
