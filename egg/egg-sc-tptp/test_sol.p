...
fof(f2, plain, [![Xx] : (cemptySet = sf(sf(sf(cemptySet))))] --> [(cemptySet = sf(sf(sf(cemptySet))))], inference(leftForall, param(0, $fot(cemptySet)), [f1])).
fof(f3, plain, [] --> [(cemptySet = sf(sf(sf(cemptySet))))], inference(cut, param(0, 0), [f2, a1])).
...
fof(f6, plain, [![Xx, Xy] : (sf(cemptySet) = sf(sf(sf(cemptySet))))] --> [(cemptySet = sf(cemptySet))], inference(leftForall, param(0, $fot(sf(cemptySet))), [f5])).
fof(f7, plain, [] --> [(cemptySet = sf(cemptySet))], inference(cut, param(0, 0), [f6, a2])).