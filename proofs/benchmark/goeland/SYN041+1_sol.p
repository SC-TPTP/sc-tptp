...
% SZS output start Proof for SYN041+1.p


fof(c_SYN041_1_p, conjecture, (~((p => q)) => (q => p))).
...
fof(f3, plain, [~((~((p => q)) => (q => p)))] --> [], inference(leftNotImplies, [status(thm), 0], [f4])).
...
% SZS output end Proof for SYN041+1.p
