...
class EGraphTerms() {
  ...
  def canonicalize(node: ENode): ENode = {
    node match
      case FunctionTerm(label, children) =>
        FunctionTerm(label, children.map(find.asInstanceOf))
      case _ => node
  }
  ...
  protected def mergeWithStep(id1: ENode, id2: ENode, step: Step): Unit = {
    if find(id1) == find(id2) then ()
      ...
      newparents
        .map { pNode =>
          val canonicalPNode = canonicalize(
            pNode
          ) // Canonicalize doesn't work: Need to do 1-step proofs
  ...
  def prove(
      ...
      base: SC.Sequent,
      stepString: String,
      thmName: String = "Congruence Proof"
  ): Option[LVL2.LVL2Proof] =
  ...
  def proveInner(
      ...
      ctx2: Seq[Formula],
      stepString: String
  ): Option[List[LVL2.LVL2ProofStep]] = {
    import sctptp.SequentCalculus as SC
    ...
    steps match
      ...
      case Some(value) =>
        ...
        if value.isEmpty then
          return Some(
            List[LVL2.LVL2ProofStep](
              SC.RightRefl(
                s"$stepString$no",
                SC.Sequent(ctx1, (id1 === id1) +: ctx2),
                0
              )
            )
          )
            ...
            (left, right) match
              case (FunctionTerm(llabel, largs), FunctionTerm(rlabel, rargs)) =>
                ...
                if !first then {
                  ...
                  val cond_id1_eq_r = SC.RightSubst(
                    name1,
                    SC.Sequent(
                      (left === right) +: ctx1,
                      (id1 === right) +: ctx2
                ...
                val step_refl = SC.RightRefl(
                  name2,
                  SC.Sequent(ctx1, (left === left) +: ctx2),
                  0
                )
                ...
                var freeCount: Int = eqs
                  .flatMap(pair =>
                    (pair._1.freeVariables ++ pair._2.freeVariables).map(_.name)
                  )
                  .foldLeft(0)((acc, x) =>
                    if x.name.no > acc then x.name.no else acc
                  )
              ...
              case _ =>
                throw new Exception(
                  "Invalid congruence proof, should be unreachable."
                )
        }
