...
private object Constants {
  ...
  def FunctionTerm(lab: String, args: List[Term]): Term = FOL.FunctionTerm(FunctionSymbol(lab, args.length), args)
  def FunctionTerm(lab: TermSymbol, args: List[Term]): Term = FOL.FunctionTerm(lab, args)
...
class EGraphTermTest extends AnyFunSuite {
  ...
  test("3 elements no congruence egraph test") {
    ...
    val id1 = egraph.add(FunctionTerm(a, List()))
    val id2 = egraph.add(FunctionTerm(b, List()))
    val id3 = egraph.add(FunctionTerm(c, List()))
  ...
  test("8 elements no congruence egraph test") {
    ...
    val id2 = egraph.add(FunctionTerm(b, List()))
    val id3 = egraph.add(FunctionTerm(c, List()))
    val id4 = egraph.add(FunctionTerm(d, List()))
    val id5 = egraph.add(FunctionTerm(e, List()))
    val id6 = egraph.add(FunctionTerm(f, List()))
    val id7 = egraph.add(FunctionTerm(g, List()))
    val id8 = egraph.add(FunctionTerm(h, List()))
    egraph.merge(id1, id2)
  ...
  test("15 elements no congruence egraph test") {
    ...
    val id7 = egraph.add(FunctionTerm(g, List()))
    val id8 = egraph.add(FunctionTerm(h, List()))
    val id9 = egraph.add(FunctionTerm(i, List()))
    val id10 = egraph.add(FunctionTerm(j, List()))
    val id11 = egraph.add(FunctionTerm(k, List()))
    val id12 = egraph.add(FunctionTerm(l, List()))
    val id13 = egraph.add(FunctionTerm(m, List()))
    val id14 = egraph.add(FunctionTerm(n, List()))
    val id15 = egraph.add(FunctionTerm(o, List()))
    egraph.merge(id1, id3)
    egraph.merge(id5, id6)
  ...
  test("4 elements withcongruence egraph test") {
    ...
    val id1 = egraph.add(FunctionTerm(a, List()))
    val id2 = egraph.add(FunctionTerm(b, List()))
    val id3 = egraph.add(FunctionTerm(F, List(id1)))
    val id4 = egraph.add(FunctionTerm(F, List(id2)))
    egraph.merge(id1, id2)
    assert(egraph.idEq(id1, id2))
  ...
  test("divide-mult-shift by 2 egraph test") {
    ...
    val egraph = new EGraphTerms()
    val one = egraph.add(FunctionTerm("one", List()))
    val two = egraph.add(FunctionTerm("two", List()))
    val a = egraph.add(FunctionTerm("a", List()))
    val ax2 = egraph.add(FunctionTerm("*", List(a, two)))
    val ax2_d2 = egraph.add(FunctionTerm("/", List(ax2, two)))
    val `2d2` = egraph.add(FunctionTerm("/", List(two, two)))
    val ax_2d2 = egraph.add(FunctionTerm("*", List(a, `2d2`)))
    val ax1 = egraph.add(FunctionTerm("*", List(a, one)))

    val as1 = egraph.add(FunctionTerm("<<", List(a, one)))
  ...
  test("long chain congruence eGraph") {
    val egraph = new EGraphTerms()
    val x = egraph.add(FunctionTerm("x", List()))
    val fx = egraph.add(FunctionTerm(F, List(x)))
    val ffx = egraph.add(FunctionTerm(F, List(fx)))
    val fffx = egraph.add(FunctionTerm(F, List(ffx)))
    val ffffx = egraph.add(FunctionTerm(F, List(fffx)))
    val fffffx = egraph.add(FunctionTerm(F, List(ffffx)))
    val ffffffx = egraph.add(FunctionTerm(F, List(fffffx)))
    val fffffffx = egraph.add(FunctionTerm(F, List(ffffffx)))
    val ffffffffx = egraph.add(FunctionTerm(F, List(fffffffx)))
...
class EGraphTermTestWithExplain extends AnyFunSuite {
  ...
  test("long chain congruence eGraph with explain") {
    ...
    assert(egraph.idEq(fx, x))
    assert(egraph.idEq(x, fx))
    assert(egraph.explain(fx, x) == Some(List(egraph.Congruence(fx, fffx), egraph.Congruence(fffx, ffffffffx), egraph.External(ffffffffx, x))))
